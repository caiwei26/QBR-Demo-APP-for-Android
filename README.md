...
**QBR (Quad Bayer Remosaic)** 是一个基于深度学习的 Android 端侧图像处理应用。本项目旨在解决 Quad Bayer 传感器在非 Binning 模式下的分辨率损失问题，通过部署轻量化但高性能的深度残差网络（Deep Residual Network），在移动端实现从 RAW 数据到高清 RGB 图像的端到端重建。与传统插值算法相比，本应用利用 **PyTorch Mobile** 和 **OpenCV**，在骁龙等移动平台上实现了 **SOTA 级别** 的重建质量（平均PSNR > 40dB），并支持无损 PNG 导出。
...
### v26.2.3 (2026-02-14)
- **⚡ 性能优化**：
    - 适度减小了模型深度，引入分块推理 (Tiled Inference) 机制，彻底解决大分辨率图像 (3MB+) 处理时的内存溢出 (OOM) 问题。
...
1.  进入本仓库的 [Releases](../../releases) 页面下载最新版 APK (`QBR_Demo_v26.2.3.apk`)。
...
本项目仅供**学术交流与研究**使用，未经作者许可不得用于商业用途。作者联系方式：2529560356@qq.com
